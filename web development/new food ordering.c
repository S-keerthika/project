...
{
	while (1) {
		printf("\n\nWelcome to Upper House Food end=' '");
...
void hotel_initialize()
{
	...
	// with The Cozy Inn and
	// some foods name and
	// their respective costs.
	strcpy(m[1].hotel, "The Cozy Inn");
	strcpy(m[1].first_food, "Sandwich");
	strcpy(m[1].second_food, "Pizza");
	strcpy(m[1].third_food, "Fried_Rice");
	strcpy(m[1].fourth_food, "Burger");
	strcpy(m[1].fifth_food, "Mojito");
	m[1].first = 70;
	m[1].second = 100;
	m[1].third = 95;
	m[1].fourth = 130;
	...
	// Food Engine and some foods name
	// and their respective costs.
	strcpy(m[2].hotel, "Food Engine");
	strcpy(m[2].first_food, "Parotta");
	strcpy(m[2].second_food, "Noodles");
	strcpy(m[2].third_food, "Prawn65");
	strcpy(m[2].fourth_food, "Barbeque");
	strcpy(m[2].fifth_food, "Nuggets");
	m[2].first = 15;
	m[2].second = 100;
	m[2].third = 99;
	m[2].fourth = 180;
	...
	// Initializing the structure with
	// Juice point and some food
	// name and their respective costs.
	strcpy(m[3].hotel, "Juice point");
	strcpy(m[3].first_food, "Kitkat shake");
	strcpy(m[3].second_food, "Miami melon");
	strcpy(m[3].third_food, "Berry Stroke");
	strcpy(m[3].fourth_food, "Grape burst");
	strcpy(m[3].fifth_food, "Cappuccino");
	m[3].first = 99;
	m[3].second = 100;
	m[3].third =99;
	m[3].fourth = 165;
...
void search_by_food()
{
	...
	while (1) {
		...
		printf("7) %s\t%d\n8) %s\t%d\n9) %s\t%d\n10) %s\t%d\n",
			...
			m[2].fourth_food, m[2].fourth,
			m[2].fifth_food, m[2].fifth);
		...
		printf("14) %s\t%d\n15) Cart\n",
			m[3].fourth_food, m[3].fourth,
			m[3].fifth_food, m[3].fifth),
...
void cart()
{
	...
	if (ch == 1) {
		...
		scanf("%d",&ch);
		if(ch==1){
			printf("your order is booked as cash on delivery\n");
		}else{
		      printf("enter card number: ");
      		  scanf("%d",&ch);
      		  printf("enter cvv: ");
      		  scanf("%d",&ch);
      		  printf("enter pin or your age: ");
      		  scanf("%d",&ch);
      		  if(ch==temp_age){
                    printf("your order booked through net banking\n");
              }else{
			       printf("your pin is wrong enter again");
			       cart();
  	          }
		      printf("\n\nThank You for your");
